 ...
 * segment (CFG_QCOM_PAS_SIGNATURE_AUTH). Builds on struct pas_mbn /
 * pas_mbn_parse() (pas_mbn_parser.h, CFG_QCOM_PAS_HASH_VERIFY), which
 * locates these regions but does not itself interpret them.
 */
 ...
 * table (the metadata blob's preamble) rather than a loaded segment. The
 * reference authenticates this entry as soon as the image signature
 * verifies, before any segment is loaded; call this at INIT_IMAGE time for
 * the same effect, rather than waiting for the per-segment check at
 * AUTH_AND_RESET.
 *
...
/*
 * pas_meta_signed_copy() - build the signed-region bytes for one signer
 * @hs:     parsed hash segment
